        ...
        for k in [1, 2, 3, 4]:
            ...
            if key in scores:
                ...
                if isinstance(bleu_val, dict):
                    bleu_val = bleu_val.get('score', list(bleu_val.values())[0])
            else:
                bleu_val = 0.0

            metrics[f"Bleu_{k}"] = torch.tensor(
                float(bleu_val), device=self.device
            )

        # --- ROUGE_L ---
        rouge_val = scores.get("rouge", {})
        rouge_L = rouge_val.get("rougeL", {})
        if isinstance(rouge_L, dict):
            rouge_L = rouge_L.get("score", list(rouge_L.values())[0])
        metrics["ROUGE_L"] = torch.tensor(
            float(rouge_L), device=self.device
        )

        # --- CIDEr ---
        cider_val = scores.get("cider", {})
        if isinstance(cider_val, dict):
            cider_val = cider_val.get("score", list(cider_val.values())[0])
        metrics["CIDEr"] = torch.tensor(
            float(cider_val), device=self.device
        )
