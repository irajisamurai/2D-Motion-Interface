...
if __name__ == '__main__':
    ...
    _ap.add_argument('--tokenizer_name', type=str, default=None)
    _ap.add_argument('--m2t_name', type=str, default=None)
    _ap.add_argument('--gpu_id', type=int, default=0)
    _known, _ = _ap.parse_known_args()

    dataset_opt_path = './checkpoints/t2m/Comp_v6_KLD005/opt.txt'

    # --- tokenizer settings: must match the model(s) in eval_text_loaders ---
    # TM2T original:  TOKENIZER_NAME='VQVAEV3_CB1024_CMT_H1024_NRES3', CODEBOOK_SIZE=1024
    # MotionGPT:      TOKENIZER_NAME='MotionGPT-base',                  CODEBOOK_SIZE=512
    # MotionGPT + 2D: TOKENIZER_NAME='MotionGPT-base-2D',               CODEBOOK_SIZE=512
    TOKENIZER_NAME = _known.tokenizer_name if _known.tokenizer_name else 'MotionGPT_2DReconstruction'
    CODEBOOK_SIZE  = 512
    _m2t_name = _known.m2t_name if _known.m2t_name else 'M2T_MotionGPT_EL4_DL4_NH8_PS'

    eval_text_loaders = {
        _m2t_name: lambda: get_text_loader(
            f'./checkpoints/t2m/{_m2t_name}/opt.txt',
    ...
    gt_loader, gt_dataset = get_dataset_text_loader(
