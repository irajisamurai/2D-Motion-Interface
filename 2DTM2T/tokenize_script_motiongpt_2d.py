...
# (ported from MotionTokenLab/TM2T/tokenize_script_motiongpt_2d.py)
# ---------------------------------------------------------------------------
...
class Motion2DTokenizeDataset(data.Dataset):
    ...
    def _create_2d_joints_from_features(self, motion):
        ...
        # remove zero-padded columns (eye/ear rows)
...
if __name__ == '__main__':
    ...
    parser.parser.add_argument(
        ...
        type=str,
        required=True,
        help='Path to trained 2D encoder checkpoint (.tar)',
    )
    opt = parser.parse()
    opt.is_train = False
    ...
    ckpt_base = torch.load(opt.motiongpt_ckpt, map_location='cpu', weights_only=True)
    base_state = {k.replace("vae.", ""): v
                  for k, v in ckpt_base["state_dict"].items() if "vae" in k}
    vqvae.load_state_dict(base_state)

    # Overwrite encoder weights with trained 2D encoder
    print(f'Loading 2D encoder from {opt.encoder_2d_ckpt}')
    ckpt_2d = torch.load(opt.encoder_2d_ckpt, map_location='cpu', weights_only=False)
    # Support both raw state_dict and checkpoints saved with a 'model_state_dict' key
    raw = ckpt_2d.get("model_state_dict", ckpt_2d)
    enc_state = {k: v for k, v in raw.items() if "encoder" in k}
    vqvae.load_state_dict(enc_state, strict=False)
    vqvae.eval()
    ...
    with torch.no_grad():
        for e in range(num_replics):
            ...
            for i, batch in enumerate(tqdm(all_loader)):
                ...
                # zero-pad 2D features to 263 dims to match VQ-VAE input width
                pad = torch.zeros(
