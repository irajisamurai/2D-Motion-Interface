...
class M2DTDataset(Dataset):
    def __init__(self, tokenizer, split='train', source_len=256, target_len=1536, unit_length=4,
        ...
        for name in tqdm(id_list):
            try:
                m_token_list = np.load(pjoin(self.data_root, token_dir_start0, '%s.npy' % name))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--token_dir_start0", type=str, default="VQVAE_start0",
                        help="Sub-directory under dataset root for frame-0-start tokens")
    ...
    train_dataset = M2DTDataset(tokenizer, split='train', token_dir_start0=args.token_dir_start0)
    val_dataset = M2DTDataset(tokenizer, split='val', token_dir_start0=args.token_dir_start0)
